...
mdt exec "sudo apt-get install -y jshon jq"
...
mdt exec sudo mkdir /home/mendel/frigate/
mdt exec "cd /home/mendel/frigate/ && wget https://raw.githubusercontent.com/simon-thorpe/editor/master/dist/editor.php"
mdt exec sudo cp OSIDS/detection/coral_devboard/frigate-config.yml /home/mendel/frigate/config.yml
mdt exec "sudo docker cp OSIDS/detection/coral_devboard/frigate2pushover.json node-red:/data/flows.json"
mdt exec "sudo docker restart node-red"
#install web browser edit
git clone https://github.com/bingen/rpi-nginx-php.git
cd rpi-nginx-php && docker build -t rpi-nginx-php .
mdt exec "sudo docker run -d --privileged --restart=unless-stopped --name nginx -v /home/mendel/frigate/:/var/www/public_html -p 80:5002 rpi-nginx-php"

#install frigate
mdt exec sudo docker run -d --privileged --restart=unless-stopped --name frigate --mount type=tmpfs,target=/tmp/cache,tmpfs-size=10000000000   --device /dev/bus/usb:/dev/bus/usb -v /home/mendel/frigate:/media/frigate   -v /home/mendel/frigate/config.yml:/config/config.yml:ro   -v /etc/localtime:/etc/localtime:ro   -e FRIGATE_RTSP_PASSWORD='password'   -p 5000:5000   -p 1935:1935   blakeblackshear/frigate:0.8.4-aarch64
mdt exec sudo touch /home/mendel/frigate/filebrowser.db
#crontab add line
mdt exec chmod a+x /home/mendel/OSIDS/detection/coral_devboard/clean.sh
#stop frigate until files are edited
mdt exec sudo docker stop frigate
#turn on ssh for mendel user for reconnecting
mdt exec sudo cp /home/mendel/OSIDS/detection/coral_devboard/sshd_config /etc/ssh/sshd_config
mdt exec sudo /etc/init.d/ssh restart
#add the start scripts
mdt exec sudo cp /home/mendel/OSIDS/detection/coral_devboard/kickstart.sh /home/mendel/kickstart.sh
mdt exec sudo cp /home/mendel/OSIDS/detection/coral_devboard/wifi.sh /home/mendel/wifi.sh
mdt exec ln -s /home/mendel/frigate/config.yml /home/mendel/config.yml
